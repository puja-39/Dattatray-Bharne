...
class SocialActivityController extends Controller
{
    ...
    public function ajax_list(Request $request)
    {
        ...
        try {
            ...
            $totalFiltered = Blog::where('name', 'LIKE', '%' . $like . '%')
                ->count();
            $datalist = Blog::where('name', 'LIKE', '%' . $like . '%')
                ->skip($start)
                ->take($limit)
        ...
        if (isset($datalist) && ! empty($datalist)) {
            foreach ($datalist as $list) {
                $edit = '';
                if (valid_session('blog', 'edit', false)) {
                    $edit = '<a href="' . route('.edit', ['id' => encode_string($list->id)]) . '" class="btn ripple btn-primary btn-sm mt-1 mb-1 text-sm text-white " data-placement="top" data-toggle="tooltip" title="' . translate('Edit') . '"> <i class="fa fa-edit"></i> </a>';
                }
                $details = '<a href="' . route('admin.blog.quick_details', ['id' => encode_string($list->id)]) . '" class="btn ripple btn-secondary btn-sm mt-1 mb-1 text-sm popup-page" data-placement="top" data-toggle="tooltip" title="' . translate('Details') . '"> <i class="fa fa-eye"></i> </a>';
        ...
        if ($request->ajax()) {
            ...
            try {
                ...
                if ($request['id'] != '') {
                    ...
                    $up_data['updated_by'] = json_encode($updated);
                    $role_id = Blog::where('id', '=', $id)->update($up_data);
                    if (empty($role_id)) {
                        DB::rollBack();
                        return response()->json([
                            'status' => $status,
    ...
    public function edit(Request $request, $id)
    {
            ...
            if ($id != '') {
                ...
                $page_data['blog'] = Blog::where('id', '=', $id)
                    ->first();
            }
            return view('backend.blog.add-edit', $page_data);
    }
            ...
            if ($request['id'] != '') {
                ...
                $up_data = array();
                  $up_data['image']               = str_replace([getBaseURL().'public/uploads/'],[''], $request['image']);
                    ...
                    if ($blogData->name !== $request['name']) {
                            $up_data['slug'] = generateSlug('blog', $request['name'], $id);
                        } else {
                            $up_data['slug'] = $blogData->slug;
                        }  
                    $up_data['short_description']   = $request['short_description'];
                    $up_data['seo_description']     = $request['seo_description'];
                    $up_data['seo_keywords']        = $request['seo_keywords'];
