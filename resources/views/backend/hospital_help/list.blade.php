@extends('backend.app')
@section('page_title'){{ translate('Hospital Help') }} @endsection
@section('style_files')
...
@section('content')
	<div class="page-header">
        <div>
            <h2 class="main-content-title tx-24 mg-b-5">{{ translate('Hospital Help') }}</h2>
            <ol class="breadcrumb">
		...
		<div class="btn btn-list">
				<a class="btn ripple popup-page btn-primary" href="{{ route('admin.hospital_help.new') }}"><i class="fe fe-plus-circle ml-2"></i> {{ translate('Add New') }}</a>
		</div>
    </div>
	...
	<script>
		$(document).ready(function () {
			$('.data-list-table').DataTable({
				...
				"serverSide": true,
				"ajax": {
					"url": "{{ route('admin.hospital_help.list') }}",
					"dataType": "json",
					"type": "POST",
					"data": {action : 'list'},
