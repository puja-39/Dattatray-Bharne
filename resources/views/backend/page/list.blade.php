...
@section('content')
	<div class="page-header">
        <div>
            <h2 class="main-content-title tx-24 mg-b-5">{{ translate('Page') }}</h2>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ translate('Dashboard') }}</a></li>
                <li class="breadcrumb-item"><a href="javascript:void(0);">{{ translate('List') }}</a></li>
            </ol>
        </div>
		@if (valid_session('page', 'add', false))
			<div class="btn btn-list">
				<a class="btn ripple btn-primary" href="{{ route('admin.page.new') }}"><i class="fe fe-plus-circle ml-2"></i> {{ translate('Add New') }}</a>
			</div>
		@endif
    </div>
