@extends('backend.app')
@section('page_title'){{ translate('Join With Us Enquiry') }} @endsection
...
@section('content')
	<div class="page-header">
        <div>
            <h2 class="main-content-title tx-24 mg-b-5">{{ translate('Join With Us') }}</h2>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ translate('Dashboard') }}</a></li>
                <li class="breadcrumb-item"><a href="javascript:void(0);">{{ translate('Enquiry') }}</a></li>
                <li class="breadcrumb-item"><a href="javascript:void(0);">{{ translate('Join With Us') }}</a></li>
            </ol>
        </div>
    </div>
	...
	<script>
		$(document).ready(function () {
			$('.data-list-table').DataTable({
				...
				"serverSide": true,
				"ajax": {
					"url": "{{ route('admin.enquiry.subscribe_list') }}",
					"dataType": "json",
					"type": "POST",
					"data": {action : 'application_list'},
				...
				"columns": [
					...
					{"data": "email", searchable: false, className: 'text-center'},
