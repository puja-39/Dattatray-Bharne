    ...
    <div class="modal-dialog modal-dialog-scrollable" role="document">
       <div class="modal-content">
          <div class="modal-header">
             <h6 class="modal-title">{{ translate('Journey Details') }}</h6>
                         ...
                         <tbody>
                           <tr>
                            ...
                            <tr>
                               ...
                               <td><span class="badge bg-{{ $journey->is_active==='1' ? 'success' : 'danger' }}">{{ $journey->is_active==='1' ? translate('Active') : translate('Inactive') }}</span></td>
                            </tr>
                            <tr>
                                <th>{{ translate('Created By') }}</th>
                                <td>{{ get_crud_user_details($journey->created_by,'name') }}</td>
                            </tr>

                            <tr>
                                <th>{{ translate('Created On') }}</th>
                                <td>{{ \Carbon\Carbon::parse($journey->created_at)->format('M d, Y h:i A') }}</td>
                            </tr>

                            <tr>
                               <th>{{ translate('Updated By') }}</th>
                               <td>{{ get_crud_user_details($journey->updated_by,'name') }}</td>
                              
                            </tr>

                            <tr>
                                <th>{{ translate('Updated On') }}</th>
                               <td>{{ \Carbon\Carbon::parse($journey->updated_at)->format('M d, Y h:i A') }}</td>
                            </tr>                       
                         </tbody>
                      </table>
                   </div>
