@extends('backend.app')
@section('page_title'){{ translate('Payment Gateway Settings') }} @endsection
...
@section('content')
	<div class="page-header">
        <div>
            <h2 class="main-content-title tx-24 mg-b-5">{{ translate('Payment Gateway Settings') }}</h2>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ translate('Dashboard') }}</a></li>
                <li class="breadcrumb-item"><a href="javascript:void(0);">{{ translate('Settings') }}</a></li>
                <li class="breadcrumb-item"><a href="javascript:void(0);">{{ translate('Payment Gateway') }}</a></li>
            </ol>
        </div>
    </div>
	...
	<script>
		$(document).ready(function () {
			$('.data-list-table').DataTable({
				...
				"serverSide": true,
				"ajax": {
					"url": "{{ route('admin.settings.payment_gateway_list') }}",
					"dataType": "json",
					"type": "POST",
					"data": {action : 'payment_gateway_list'},
				...
				"columns": [
					...
					{"data": "description", searchable: false, className: 'text-center'},
